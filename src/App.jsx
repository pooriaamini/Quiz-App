...
function reducer(state, action) {
  ...
  switch(action.type){
    ...
    const {sub,num}=action.payload
      ...
      return{
        ...state,subjectOfQuiz:sub!==false ? sub : 'react',

        numberOfQuiz:num!==false ? num :state.numberOfQuiz,status:'saveData'
      }
    ...
    return{
      ...
      ...state,questions:action.paylaod,
      status:'saveData'
...
function App() {
  ...
  const [{questions,status,index,select,point,subjectOfQuiz,numberOfQuiz}, dispatch] = useReducer(reducer, initialstate)
...
<Header/>
      ...
      {status==='started' && <StartScreen subjectOfQuiz={subjectOfQuiz}   maxPoint={maxPoint} numQuestions={numberOfQuiz} point={point} index={index} select={select} dispatch={dispatch}  questions={questions[index]}/>}
